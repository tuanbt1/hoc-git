...
class RedshopModelAccount extends RedshopModel
{
	...
	public function removeWishlistProduct()
	{
		...
		$productDetail     = RedshopHelperProduct::getProductById($pid);
		$user = JFactory::getUser();
		...
		if (count($list) > 0)
		{
			...
			if ($db->execute())
			{
				$app->enqueueMessage(\JText::sprintf('COM_REDSHOP_WISHLIST_PRODUCT_DELETED_SUCCESSFULLY',$productDetail->product_name));
			}
			else
			{
