		...
		switch ($pluginType)
		{
			...
			case Sh404sefClassBaseextplugin::TYPE_JOOMLA_ROUTER:
				...
				*/
				...
				if (($insertMenuIdForRouterPhp || empty($title)) && isset($originalVars['Itemid']) && !empty($originalVars['Itemid']))
				{
					$menu = JFactory::getApplication()->getMenu();
					$item = $menu->getItem($originalVars['Itemid']);

					if (is_object($item) && $originalVars['option'] == $item->component)
					{
				...
				{
					$title[] = substr($originalVars['option'], 4);
				}
