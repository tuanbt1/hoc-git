jQuery(document).ready(function ($) {
    ...
    $('.redshop_newsletter').find('.redshop_newsletter_email').each(function(i,e) {
        $(this).attr("placeholder","e-mail của bạn");
        $(this).focus(function(event) {
            $(this).attr("placeholder","");
        });
        $(this).blur(function(event) {
...
/*    $('.product_attribute select').select2OptionPicker();
    ...
    $('.attribute_property .select-buttons').find('li:first-child').each(function(index, val) {
        ...
        if($(this).find('>a').html().toUpperCase()=='CHỌN MÀU'){//this name must be same with innerhtml element
            $(this).parent().addClass('box-color');
            $(this).addClass('box-color-title');
            $(this).find('>a').html('Màu:');
        }
        if($(this).find('>a').html().toUpperCase()=='CHỌN KÍCH THƯỚC'){//this name must be same with innerhtml element
            $(this).parent().addClass('box-size');
            $(this).addClass('box-size-title');   
            $(this).find('>a').html('Kích thước:');
            $(this).parent().parent().parent().after('<div class="size-guide"><a href="#">Hướng dẫn kích thước</a></div>');
        }
        if($(this).find('>a').html().toUpperCase()=='SELECT SIZE'){//this name must be same with innerhtml element
            $(this).parent().addClass('box-size');
            $(this).addClass('box-size-title');   
    ...
    $('.attribute_property .box-color li').find('>a').each(function(index, val) {
        var _color = $(this).html();
        var temp_classname = null;
        if(_color == 'đỏ'){
            temp_classname= 'red';
            $(this).css("background-color", "red");
        }else if(_color == 'nâu'){
            temp_classname= 'brown';
            $(this).css("background-color", "brown");
        }else if(_color == 'cam'){
            temp_classname= 'orange';
             $(this).css("background-color", "orange");
        }else if(_color == 'xanh dương'){
            temp_classname= 'blue';
  ...
  /*  $('.attribute_property .box-color li').find('>a').each(function(index, val) {
        var _color = $(this).attr('data-select-index');
        var temp_classname = null;
        if(_color == 1){
            temp_classname= 'blue';
        }else if(_color == 2){
...
jQuery(document).ready(function($){
    var widthscreen=767;
    var temppos=$('#grid-tab .mod_redshop_products_wrapper');
    var temppos_2=$('#grid-bottom .mod_redshop_products_wrapper');
    var menu_main=$('.topbar');
    var temp=0, count_1=0;
    $( document ).ready(function() {
        if ($(window).width()<1200) {      
             $('#header #top #top_primary .modlogin .menu').hover(function(){
                $('#header #top #top_primary .modlogin .menu > .btn-logout').toggle();
            });   
        };
    });
    $( document ).ready(function() {
        if ($(window).width()<= widthscreen) { 
            ...
            $('.menu_smp').click(function(){
                menu_main.toggle();
                if(temp==0){
                    $('#featured').css('opacity','0.6');
                    temp=1;
                }
                else{
    ...
    var footer_1= $('#footer .mod_1 h3');
    var footer_2= $('#footer .mod_2 h3');
    var footer_3= $('#footer .mod_3 h3');
    var footer_1_menu= $('#footer .mod_1 >ul');
    var footer_2_menu= $('#footer .mod_2 >ul');
    var footer_3_menu= $('#footer .mod_3 >ul');
    var temp_1=0,temp_2=0, temp_3=0;
  
    $( document ).ready(function() {
        if ($(window).width()<= widthscreen) {
            footer_1.click(function(){
                footer_1_menu.toggle();
                if(temp_1%2==0){
                    $('#footer .mod_1 h3').css('background-image',"url('/images/menu_footer_5.png')");
                    temp_1+=1;
                }
                else{
                    $('#footer .mod_1 h3').css('background-image',"url('/images/menu_footer_3.png')");
                    temp_1=0;
                }
            });
           
            footer_2.click(function(){
                footer_2_menu.toggle();
                if(temp_2%2==0){
                    $('#footer .mod_2 h3').css('background-image',"url('/images/menu_footer_5.png')");
                    temp_2+=1;
                }
                else{
                    $('#footer .mod_2 h3').css('background-image',"url('/images/menu_footer_3.png')");
                    temp_2=0;
                }
            });
            footer_3.click(function(){
                footer_3_menu.toggle();
                if(temp_3%2==0){
                    $('#footer .mod_3 h3').css('background-image',"url('/images/menu_footer_5.png')");
                    temp_3+=1;
                }
                else{
                    $('#footer .mod_3 h3').css('background-image',"url('/images/menu_footer_3.png')");
                    temp_3=0;
                }          
            });

            $('.product_search').hover(function(){
                $('#keyword').toggle();
            });
            ...
            $('#nav-menu-mobile .item-148').hover(function(){
                $('#nav-menu-mobile .item-148 >ul >li').toggle();
            });
            $('#nav-menu-mobile .item-114').hover(function(){
                $('#nav-menu-mobile .item-114 >ul >li').toggle();
            });
            $('#nav-menu-mobile .item-182').hover(function(){
                $('#nav-menu-mobile .item-182 >ul >li').toggle();
            });
              $('#nav-menu-mobile .item-176').hover(function(){
                $('#nav-menu-mobile .item-176 >ul >li').toggle();
            });
            $('#nav-menu-mobile .item-149').hover(function(){
                $('#nav-menu-mobile .item-149 >ul >li').toggle();
            });
             $('#nav-menu-mobile .item-185').hover(function(){
                $('#nav-menu-mobile .item-185 >ul >li').toggle();
            });
            $('#nav-menu-mobile .item-160').hover(function(){
                $('#nav-menu-mobile .item-160 >ul >li').toggle();
            });
              $('#nav-menu-mobile .item-165').hover(function(){
                $('#nav-menu-mobile .item-165 >ul >li').toggle();
            });
    
        };
    });
     $('#redshopcomponent .btn-shopmore input').attr('onclick', "document.location='/redshop/categories';"); 
     $(".login .fotgotpw a").attr("href", "/login?view=reset");
     $('#divOnestepCheckout .panel-body > a').click(function(){
        $('#sbox-content .contentpane .account-tab .tab-link').css('color','#e2b870 !important');
     });
});
