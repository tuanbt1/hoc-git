...
$customer_username_lbl = '';
$customer_username     = '';

if ($user->username != "")
{
	$customer_username_lbl = 'Username';
	$customer_username     = $user->username;
}

$template_desc = str_replace('{username_lbl}', $customer_username_lbl, $template_desc);
$template_desc = str_replace('{username}', $customer_username, $template_desc);
...
$customer_firstname_lbl = '';
$customer_firstname     = '';

if ($this->userdata->firstname != "")
{
	$customer_firstname_lbl = JText::_('COM_REDSHOP_CUSTOMER_FULLNAME');
	$customer_firstname     = $this->userdata->firstname;
}

$template_desc = str_replace('{firstname_lbl}', $customer_firstname_lbl, $template_desc);
$template_desc = str_replace('{firstname}', $customer_firstname, $template_desc);

$customer_lastname_lbl = '';
$customer_lastname     = '';

if ($this->userdata->lastname != "")
{
	$customer_lastname_lbl = JText::_('COM_REDSHOP_CUSTOMER_FULLNAME');
	$customer_lastname     = $this->userdata->lastname;
}

$template_desc = str_replace('{lastname_lbl}', $customer_lastname_lbl, $template_desc);
$template_desc = str_replace('{lastname}', $customer_lastname, $template_desc);
...
if (strstr($template_desc, "{quotation_loop_start}") && strstr($template_desc, "{quotation_loop_end}"))
{
	...
	if (count($quotationlist))
	{
		for ($j = 0; $j < count($quotationlist); $j++)
		{
			...
			if($quotationlist[$j]->quotation_status=='1'){ $quotation_status= 'confirm';} else $quotation_status='not confirm';

			$quotation_data = str_replace('{quotation_index}', JText::_('COM_REDSHOP_QUOTATION') . " ".$quotationlist[$j]->quotation_id, $quotation_data);
			...
			$quotation_data = str_replace('{quotation_date}', date('Y-m-d', $quotationlist[$j]->quotation_cdate), $quotation_data);
			$quotation_data = str_replace('{quotation_status}', $quotation_status, $quotation_data);
		}
	}
	else
	{
