...
$login_template_desc = "
<div class='username'><label>{rs_username_lbl}:</label>{rs_username}</div>
<div class='password-box'><label class='password'>{rs_password_lbl}:</label>{rs_password}</div>
<div class='forgot-pass'>{forget_password_link}</div>
<div class='loginbutton'>{rs_login_button}</div>
";
?>
<div class="checkout-bar">
	<ul>
	<li class="checkout-bar-1-active"><?php echo JText::_('COM_REDSHOP_ORDER_INFORMATION');?></li>
	<li class="checkout-bar-2"><?php echo JText::_('COM_REDSHOP_PAYMENT');?></li>
	<li class="checkout-bar-3"><?php echo JText::_('COM_REDSHOP_RECEIPT'); ?></li>
	</ul>
</div>
...
else
{
	...
	if ($show_login)
	{
		...
		$forgotpwd_link      = JRoute::_('index.php?option=com_users&view=reset&Itemid=' . $Itemid);
		...
		$login_template_desc = '<div class="mystretchermy" id="login_div" ' . $loginuserstyle . '><h2 class="title">'.JText::_('COM_REDSHOP_EXISTCUSTOMER').'</h2><form action="' . JRoute::_('index.php') . '" method="post">' . $login_template_desc . '</form></div>';
	...
	<div class="mystretchermy" id="register_div" <?php echo $newuserstyle;?>>
		<h2 class="title"><?php echo JText::_('COM_REDSHOP_NEWCUSTOMER');?></h2>
		<div class='regtype'>
			<div class='pri_person' <?php echo $allowCustomer;?>>
	        ...
	        <div class='business' <?php echo $allowCompany;?>>
			...
			if (count($telesearch) > 0 && $telesearch[0]->enabled)
			{
				...
				<tr>
					<td colspan="2">
                        <?php echo JText::_('COM_REDSHOP_GET_ADDRESS_BY_PHONE') . ':<input name="searchphone" id="searchphone" type="text" value="" /><input type="button" name="searchaddbyphone" id="searchaddbyphone" value="' . JText::_('COM_REDSHOP_SEARCH') . '" onclick="return searchByPhone();" />';?>
						<br/>
						<div id="divSearchPhonemsg" style="display:none"><?php echo JText::_('COM_REDSHOP_NO_RESULT_FOUND_BY_SEARCHPHONE');?></div>
					</td>
				</tr>
			<?php
			}
		...
		<form action="<?php echo JRoute::_('index.php'); ?>" method="post" name="adminForm" id="adminForm"
			...
			if (REGISTER_METHOD == 2)
			{
				...
				<h4><input type="checkbox" name="createaccount" <?php echo $checked_style;?> id="createaccount"
					...
					<label for="createaccount"><?php echo JText::_('COM_REDSHOP_CREATE_ACCOUNT');?></label></h4>
			<?php
			}    ?>
			<div>
