...
if ($total > 0)
{
	...
	foreach ($list as $data)
	{
		...
		if (strstr($template, "{stock_status"))
		{
			...
			$productStockStatus = $producthelper->getproductStockStatus($product->product_id);
			...
			if (!$productStockStatus['regular_stock'])
			{
				...
				{
					$stock_status = "<span id='stock_status_div" . $product->product_id . "'><div id='" . $out_stock_class
						. "' class='" . $out_stock_class . "'>" . JText::_('COM_REDSHOP_OUT_OF_STOCK') . "</div></span>";
				}
				else
				{
					$stock_status = "<span id='stock_status_div" . $product->product_id . "'><div id='" . $pre_order_class
			...
			{
				$stock_status = "<span id='stock_status_div" . $product->product_id . "'><div id='" . $avail_class
					. "' class='" . $avail_class . "'>" . JText::_('COM_REDSHOP_AVAILABLE_STOCK') . "</div></span>";
			}

			$template = str_replace('{stock_status}', $exp_div . $stock_status . $div_end . $td_end . $td_start . '{stock_status}', $template);
		}
	...
	$template = str_replace('{stock_status}', "", $template);
