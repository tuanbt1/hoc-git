...
// Facebook I like Button2
if (strstr($template_desc, "{facebook_like_button2}"))
{
	$uri           = JFactory::getURI();
	$facebook_link = urlencode($uri->toString());
	$facebook_title = $this->escape($this->data->product_name);
	$facebook_like = '<a target="_blank" href="http://www.facebook.com/share.php?u='.$facebook_link.'&title='.$facebook_title.'"><img src="templates/redshoemaniac/images/icon_fa.png" width="36" height="24"/></a>';
	$template_desc = str_replace("{facebook_like_button2}", $facebook_like, $template_desc);
...
// twitter button
if (strstr($template_desc, "{twitter_button}"))
{
	$uri           = JFactory::getURI();
	$link_sc = urlencode($uri->toString());
	$title_sc = $this->escape($this->data->product_name);
	$like_sc = '<a target="_blank" href="http://twitter.com/home?status='.$link_sc.'+'.$title_sc.'"><img src="templates/redshoemaniac/images/icon_twitter.png" width="36" height="24"/></a>';
	$template_desc = str_replace("{twitter_button}", $like_sc, $template_desc);
...
// pinterest button
if (strstr($template_desc, "{pinterest_button}"))
{
	$uri           = JFactory::getURI();
	$link_sc = urlencode($uri->toString());
	$title_sc = $this->escape($this->data->product_name);
	$like_sc = '<a target="_blank" href="http://pinterest.com/pin/create/bookmarklet/?media='.$link_sc.'&url='.$link_sc.'&is_video=false&description='.$title_sc.'"><img src="templates/redshoemaniac/images/icon_pinterest.png" width="36" height="24"/></a>';
	$template_desc = str_replace("{pinterest_button}", $like_sc, $template_desc);
...
if (strstr($template_desc, "{googleplus_button}"))
{
	$uri           = JFactory::getURI();
	$link_sc = urlencode($uri->toString());
	$title_sc = $this->escape($this->data->product_name);
	$like_sc = '<a target="_blank" href="https://plus.google.com/share?url='.$link_sc.'"><img src="templates/redshoemaniac/images/icon_ggplus.png" width="36" height="24"/></a>';
	$template_desc = str_replace("{googleplus_button}", $like_sc, $template_desc);
...
if ($user->id)
{
	...
	if (strstr($template_desc, "{form_rating_frame}"))
	{
		$reviewlink    = "";
		$reviewform    = "";
		$reviewlink    = JURI::root() . 'index.php?option=com_redshop&view=product_rating&tmpl=component&product_id=' . $this->data->product_id .
										'&category_id=' . $this->data->category_id .
										'&Itemid=' . $this->itemId;
		$reviewform    = '<iframe src="'.$reviewlink.'" frameborder="0" width="620" height="420"></iframe>';
		$template_desc = str_replace("{form_rating_frame}", $reviewform, $template_desc);
	}
}
else
{
...
if (strstr($template_desc, "{product_rating}"))
{
	...
	if ($product_template != "" && $reviews_template != "" && count($reviews) > 0)
	{
		...
		for ($j = 0; $j < $mainblock && $j < count($reviews); $j++)
		{
			...
			$reviews_data1 = str_replace("{review_date}", date('D, d/m/Y',$reviews [$j]->time), $reviews_data1);
		...
		if ($mainblock < count($reviews))
		{
			...
			$reviews_data .= '<img src="' . REDSHOP_MEDIA_IMAGES_ABSPATH . 'reviewarrow.gif"> ';
...
if (strstr($template_desc, "{ask_question_about_product}"))
{
	...
	$ask_question_link = '<a class="redbox" rel="{handler:\'iframe\',size:{x:500,y:320}}" href="' . $asklink . '" >' .
