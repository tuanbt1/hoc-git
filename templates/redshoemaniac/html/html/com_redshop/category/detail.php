...
if (!$slide)
{
	...
	$pro_total = $model->_total;
	$pro_per_page = $this->maincat->products_per_page;
	$pro_start = 1;
	$pro_end = 0;
	if (JRequest::getVar('start')) {
		$pro_start = JRequest::getVar('start')+1;
	}
	$pro_end = $pro_start+$pro_per_page;
	if ($pro_end>$pro_total) {
		$pro_end = $pro_total;
	}
	if ($pro_start > $pro_total) {
		$pro_start = $pro_total;
	}
	$total_string = $pro_start.'-'.$pro_end.' of '.$pro_total;
	$template_desc = str_replace("{total_product}", $total_string, $template_desc);
