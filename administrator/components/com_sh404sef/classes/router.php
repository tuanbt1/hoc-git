...
class Sh404sefClassRouterInternal extends JRouterSite
{
	...
	public $joomlaExceptionHandler = null;
	...
	protected function _checkDomain($originalUri)
	{
		...
		// Redirect if needed
		if ($shouldRedirect && !empty($targetUrl))
		{
	...
	protected function _setupNotFoundErrorHandling()
	{
		...
		$this->joomlaExceptionHandler = set_exception_handler(array($this, 'sh404sefExceptionHandler'));
	}
	...
	protected function _autoRedirect404($uri)
	{
		if (function_exists('shFindSimilarUrls'))
		{
			...
			/**
			 * Whether 404 auto-redirect should be attempted for the current request.
			 *
			 * @api
			 * @package sh404SEF\filter\router
			 * @var sh404sef_should_try_404_auto_redirect
			 * @since   4.17.1
			 *
			 * @param bool   $shouldAutoredirect If false, do not attempt to find similar URLs.
			 * @param string $path The requested path.
			 * @param JUri   $uri The incoming URI.
			 *
			 * @return array
			 */
			$shouldAutoredirect = ShlHook::filter(
				'sh404sef_should_try_404_auto_redirect',
				true,
				$path,
				$uri
			);
			...
			if (!empty($urlList))
			{
				...
				if (!empty($target->oldurl) && $path != $target->oldurl && $this->_canRedirectFrom($uri, $method = '', $path))
				{
					...
					/**
					 * Filter the target URL and message before performing a
					 * 404 autoredirect.
					 *
					 * @api
					 * @package sh404SEF\filter\router
					 * @var sh404sef_404_auto_redirect_target
					 * @since   4.17.1
					 *
					 * @param array  $target An array with 'url' and 'message' items.
					 * @param string $path The requested path.
					 * @param JUri   $uri The incoming URI.
					 *
					 * @return array
					 */
					$filteredTarget = ShlHook::filter(
						'sh404sef_404_auto_redirect_target',
						$target,
						$path,
						$uri
					);
	...
	public function sh404sefExceptionHandler($exception)
	{
		...
		if (!empty($this->joomlaExceptionHandler))
		{
			call_user_func_array(
				$this->joomlaExceptionHandler,
				array($exception)
			);
		}
	}
	...
	public function sh404sefErrorPage($error)
	{
		...
		if (404 != $code)
		{
			// do not handle other, non-404 exceptions
			return;
		}
